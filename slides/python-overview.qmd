...
```{mermaid}
flowchart LR
    P["Python Code"] -->|"interpreted"| M["Python VM<br/>(bytecode)"]
    P -->|"calls"| C["C / Fortran<br/>Libraries"]
    C -->|"precompiled"| M
```
:::
...
- **Interpreted:** Python compiles your code to bytecode, which the Python runtime executes — flexible, but slower than native code
- **Precompiled:** Libraries like pandas and NumPy are written in C/Fortran, compiled to machine code *ahead of time* — called by Python, runs at near-native speed
:::
...
::: {.highlight-box}
Same code — different containers. Notebooks let you run one piece at a time and inspect the result. We'll use Google Colab (a free notebook environment) today.
:::
...
::: {.info-box}
[Your notebook environment today]{.box-title}

- Free, runs in your browser — nothing to install
- Go to **colab.research.google.com**
- Open the companion notebook (link on course page)
- **Shift + Enter** — run a cell and move to the next
- **Ctrl + Enter** — run a cell and stay on it
:::

::: {.highlight-box}
We'll use Colab in the second half of the session. For now, follow along on the slides.
:::
...
1. Open the companion notebook in Google Colab
2. Click the **Gemini** sparkle icon (top right) — or use a new code cell
3. Type your prompt describing what you want Python to do
4. Gemini generates code — click **Insert** to add it to a cell
5. Run the cell with **Shift + Enter**
:::
...
::: {.info-box}
[Type this into Gemini]{.box-title}
