...
%%| fig-width: 9
%%| fig-height: 1.8
%%{init: {'themeVariables': {'fontSize': '16px', 'fontFamily': 'Inter, sans-serif'}, 'flowchart': {'diagramPadding': 16}}}%%
flowchart LR
...
```{=html}
<script>
/* Set SVG to exactly 1:1 scale (CSS width = viewBox width) so foreignObject boxes
   exactly fit the measured text. The CSS transform: scale(1.4) on .mermaid-sm then
   scales the whole diagram up uniformly — including the CSS text inside foreignObjects. */
window.addEventListener('load', function() {
  setTimeout(function() {
    var svg = document.querySelector('.mermaid-sm svg.mermaid-js');
    if (svg) {
      var vb = svg.getAttribute('viewBox');
      if (vb) {
        var parts = vb.trim().split(/\s+/);
        svg.setAttribute('width', String(Math.ceil(parseFloat(parts[2]))));
        svg.setAttribute('height', String(Math.ceil(parseFloat(parts[3]))));
      }
